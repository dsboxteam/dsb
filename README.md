...
It provides a user-friendly interface for accessing containers
when developing software, and simplifies the everyday use of containerized
applications.
...
Using the [`dsb sh`](#dsb-sh) and [`dsb root`](#dsb-root) subcommands without specifying
container's commands simply run the shell of the corresponding container.
If possible, the current working directory is mapped to the container's working directory.

For more convenience, Dsb supports execution of frequently used
container commands via [custom `dsb` subcommands](#custom-subcommands)
and [Dsb scripts](#dsb-scripts). The latter can be called
identically to the container's native commands,
...
* [Reference](#reference)
    ...
    * [Dsb project variables](#dsb-project-variables)
        * [Base variables](#base-variables)
            * [`DSB_ROOT`](#base-variables)
            * [`DSB_BOX`](#base-variables)
            * [`DSB_UTILS`](#base-variables)
            * [`DSB_SKEL`](#base-variables)
            * [`DSB_UID`](#base-variables)
            * [`DSB_GID`](#base-variables)
            * [`DSB_UID_GID`](#base-variables)
        * [Variables used in `.dsbenv` file](#variables-used-in-dsbenv-file)
...
* __service__  
This [Compose term](https://docs.docker.com/compose/compose-file/05-services/)
refers to a container or a group of containers
with one shared configuration labeled with a particular name
(further referred to as _service name_).
The term can be regarded as a synonym for the term _container_
unless we are talking about _scaled services_.

* __scaled service__  
The _service_ with several running containers.
See [`dsb scale`](#dsb-scale) and [Container indexes.](#container-indexes)

* __Dsb project__  
[Compose project](https://docs.docker.com/compose/features-uses/#have-multiple-isolated-environments-on-a-single-host)
...
See [Dsb project structure](#dsb-project-structure) for more information.  
...
The host system's directory containing the `.dsb` subdirectory
...
\---  
Note: _Dsb root directory_ is, in a sense, similar to a _Vagrant project directory_.

* __mounted directory__  
A directory on a host system that accessible to one or more containers via
[bind mounts](https://docs.docker.com/storage/bind-mounts/).
There can be any number of _mounted directories_ on a host system.
They can overlap. See [Mounted directories.](#mounted-directories)

* __.dsb directory__  
The child subdirectory of the Dsb root directory where Dsb project’s configuration is stored.
It containes a `.dsb/compose` subdirectory with the `.dsbenv` file and `yaml files`,
and several subdirectories with containers' persistent data (configuration files, logs, etc.).
The `.dsb` directory is initialized via [`dsb init`](#dsb-init) subcommand.
See [.dsb directory](#dsb-directory) for more information.

* __.dsbenv file__  
The Bash source file that contains [Dsb project variables](#variables-used-in-dsbenv-file)
and [Compose variables](https://docs.docker.com/compose/environment-variables/envvars/)
and performs the same role as a 
[Compose environment file.](https://docs.docker.com/compose/environment-variables/env-file/)
In Dsb, the file is placed in the `.dsb/compose` subdirectory.
See [.dsbenv file](#dsbenv-file) for details.

* __yaml file__  
[Compose YAML file](https://docs.docker.com/compose/compose-yaml-file/)
that normally contains the configuration of a particular _service_.
In Dsb, _yaml files_ are placed in the `.dsb/compose` subdirectory.
The list of active _yaml files_ is assigned manually to
[`COMPOSE_FILE`](https://docs.docker.com/compose/environment-variables/envvars/#compose_file)
variable in the `.dsb/compose/.dsbenv` file.
See [Yaml files](#yaml-files) and [`dsb yaml`](#dsb-yaml) subcommand.

* __dsbuser__  
A special container's account with the same `UID` and `GID` as the _host user's_ ones.
See [`dsbuser` account.](#dsbuser-account)

* __Dsb script__  
An executable Bash script that containes shebang string
`#!/usr/bin/env dsb-script` and calls to
...
volumes:
  - $DSB_SPACE:/dsbspace
```
(see [`DSB_ROOT`](#base-variables) and
...
with [project variables](#variables-used-in-dsbenv-file).
...
of internal container processes.
...
automatically create subdirectories in the `.dsb/home` and `.dsb/logs` directories
with the names of the Dsb project services.
So, you just need to add the appropriate bind-mounts directives
to the [yaml files](#yaml-files) created in the `.dsb/compose` directory.
Examples of such directives are added as comments to the [yaml files](#yaml-files)
generated by the [`dsb yaml`](#dsb-yaml) subcommand.

By default, before starting the containers
the [`dsb start`](#dsb-start) and [`dsb restart`](#dsb-restart) subcommands
assign `a=rwx` access permissions to service subdirectories of the `.dsb/logs` directory.
That's because the main container workload can be run under arbitrary container's account.
Also, for security reasons, `u=rwx,go-rwx` access permissions are assigned
to service subdirectories of the `.dsb/home` directory.

If you wish, you can disable the presetting of access permissions for already existing subdirectories
by specifying [`DSB_PROD_MODE=true`](#dsb_prod_mode) in the [`.dsbenv` file](#dsbenv-file).


#### `.dsbenv` file

The `.dsbenv` file is used to define [Dsb configuration variables](#variables-used-in-dsbenv-file).
...
Then you just need to adjust the values of the [Dsb variables](#variables-used-in-dsbenv-file)
...
* [`COMPOSE_...`, `DOCKER_...`](https://docs.docker.com/compose/reference/envvars/) - Compose and Docker environment variables
...
* [`DSB_SERVICE_...`](#dsb_service_) - specific service names for [Dsb scripts](#dsb-scripts)
* [`DSB_SHUTDOWN_TIMEOUT`](#dsb_shutdown_timeout) - shutdown timeout for containers
* [`DSB_SPACE`](#dsb_space) - the full path to the [mounted directory](#mounted-directories) (see [`dsb yaml`](#dsb-yaml))
...
* When assigning values to configuration variables, you can use [Dsb base variables](#base-variables).
...
You should never explicitly spicify Compose project name.
...
must always include `globals.yaml`. This file is created by the [`dsb init`](#dsb-init) subcommand
and containes the project's network settings.
...
For other details, see [Variables used in `.dsbenv` file](#variables-used-in-dsbenv-file).
...
[multiple yaml files](https://docs.docker.com/compose/reference/#specifying-multiple-compose-files).
Dsb uses the "multiple" option, where each single service definition is stored in a separate yaml file.

All yaml files of the Dsb project are stored in the `.dsb/compose` directory - the same place
where the [`.dsbenv`](#dsbenv-file) file is stored. The list of available yaml files is specified
in the [`.dsbenv`](#dsbenv-file) file using the [`COMPOSE_FILE`](#compose_file) variable.
...
subcommand [`dsb yaml`](#dsb-yaml). This subcommand automatically
adds to the yaml file all helper elements necessary for execution 
`dsb sh`, `dsb root` subcommands and [Dsb scripts](#dsb-scripts).
...
use the [`DSB_ROOT`](#base-variables) variable.
...
See also:
* [Dsb variables used in yaml files](#variables-used-in-yaml-files)
* [COMPOSE_FILE variable](https://docs.docker.com/compose/environment-variables/envvars/#compose_file)
* [Compose Specification](https://docs.docker.com/compose/compose-file/)


### Mounted directories

A mounted directory is a host system's directory
exposed to one or more containers via
[bind mounts](https://docs.docker.com/storage/bind-mounts/).
...
If host user's files handled in the container are in the same mounted directory,
the `/dsbspace` mount point is typically used for that directory.
This mount point is not mandatory - it is just added by default
to the [yaml file](#yaml-files) when configuring a new service using
the [`dsb yaml`](#dsb-yaml) subcommand. You can then remove or change it.

There are no strict restrictions on the positions of mounted directories on a host system,
but taking into account the [`dsb`](#dsb-utility) utility's search rules for Dsb root directories,
...
Such scripts can be successfully run from any directory,
and the mounted directories of the corresponding Dsb projects can reside
anywhere on the host system.

In addition to host user's files, the data to be mounted in containers may include
internal persistent data of the containers.
...
To persist other internal data, it is preferable to use
[Compose named volumes](https://docs.docker.com/compose/compose-file/07-volumes/).

See also
[Executing commands in containers](#executing-commands-in-containers),
[`dsb yaml`](#dsb-yaml), [`DSB_SPACE`](#dsb_space).
...
The Dsb project configuration is searched
from the current working directory up through the chain of parent directories
until it is found a Dsb root directory.
To address subcommands to different Dsb projects, you should run them
...
* The [`dsb sh`](#dsb-sh) subcommand executes container's commands with permissions
...
but executes container's commands with `root` permissions.
...
frequently performed actions specific to particular Dsb projects.
The subcommand files are stored in the `.dsb/bin` directories of the projects.
...
The mapping option is enabled only when working with services
specified in the [`DSB_ARGS_MAPPING`](#dsb_args_mapping) variable
of the [`.dsbenv`](#dsbenv-file) file.

> The mapping option can be useful when integrating Dsb scripts with IDE settings
...
The following characters are considered as substring limiters:
`space`, `=`, `:`, `,`, `;`, `>`, `<`, `"`, `'`, \`, `(`, `)`.
...
stored in the project's `.dsb/bin` directory.
* `...PARAMETERS` are parameters that are passed to the script or executable on invocation.

The `dsb` utility checks for a file with the corresponding name in the `.dsb/bin` directory
if the first parameter of the `dsb` command doesn't match any of the built-in subcommands.
...
* In the absence of the `execute` permission a Bash source file is implied.
The execution of such a source file follows the same rules as the execution of
[Dsb scripts](#dsb-scripts).
The only differences are: a) no shebang string is required,
and b) the [`dsb_set_box`](#dsb_set_box) function has already been called
when executing the source file.

The Bash source file or executable has access to all [Dsb project variables](#dsb-project-variables).
The current working directory when running the Bash file or executable
is the same one where the `dsb` command is invoked.

Bash source file example:

    echo "Subcommand parameters: ${@}"
    echo "Current working directory: $PWD"
    echo "Dsb root directory: $DSB_ROOT"
    echo ".dsb directory:  $DSB_BOX"
    echo "DSB_PROJECT_ID: $DSB_PROJECT_ID"
    echo "COMPOSE_PROJECT_NAME: $COMPOSE_PROJECT_NAME"
    echo "Running containers:"
    dsb_docker_compose ps
...
The subcommand executes container commands with permissions
...
* The [`DSB_SPACE`](#dsb_space) variable is used in yaml templates
generated by the [`dsb yaml`](#dsb-yaml) subcommand.
There it represents a commmon [mounted directory](#mounted-directories) of the Dsb project.
The variable is provided just for convenience and is not used anywhere else.
The corresponding line of the [yaml file](#yaml-files) is not mandatory and can be removed.  
The default variable value is the same as the `DSB_ROOT` value.
For security reasons, it is not recommended to bind-mount the Dsb root directory directly in containers.
Please store [mounted directories](#mounted-directories) in some subdirectories of the Dsb root directory.
Appropriate adjustment to the `DSB_SPACE` variable value can be done in the [`.dsbenv`](#dsbenv-file) file.
...
It contains the list of [yaml files]((#yaml-files)) of the Dsb project.

Filenames in the list are separated by colons:

     COMPOSE_FILE="globals.yaml:py.yaml"

The `globals.yaml` file must always be present.
...
So, different Dsb projects must have different lowercase values of the `DSB_PROJECT_ID` variables on the same host system. 
...
subcommand and specifies the common [mounted directory](#mounted-directories)
...
The use of `DSB_SPACE` variable is optional - it's just provides a way to specify a certain
directory as common [mounted directory](#mounted-directories).

The default value of `DSB_SPACE` variable is the same as the value of 
[`DSB_ROOT`](#base-variables) variable,
...
If the mounted directory is in the Dsb root directory, it is recommended
to set the value relative to `DSB_ROOT` variable.
Example:

    DSB_SPACE="$DSB_ROOT/src"

See also [Mounted directories](#mounted-directories).
...
[Dsb scripts](#dsb-scripts),
and in OS commands invoked by the `dsb` utility and [Dsb scripts](#dsb-scripts).
...
[Dsb base variables](#base-variables) and 
[`.dsbenv` file variables](#variables-used-in-dsbenv-file)
become available to the Dsb script after explicitly calling the 
[`dsb_set_box`](#dsb_set_box) or [`dsb_set_single_box`](#dsb_set_single_box) functions,
or after calling any Dsb function that implicitly invokes the [`dsb_set_box`](#dsb_set_box) function.

Some [Dsb functions](#functions-available-in-dsb-scripts) return output data
in variables prefixed with `DSB_OUT_`.
Note that the values of the `DSB_OUT_...` variables may be destroyed
by subsequent calls to any Dsb functions.
So, variable values should be saved in local script variables for their further use.
...
In [yaml files](#yaml-files) you can use all the [Dsb base variables](#base-variables)
...
See also [Dsb base variables](#base-variables),
...
[Dsb scripts](#dsb-scripts),
[Service aliases](#service-aliases),
and [Custom subcommands](#custom-subcommands).
...
is printed to STDERR and the script is terminated.
...
The function executes the specified command in the container with the `root` account permissions.
...
with the [`dsbuser`](#dsbuser-account) account permissions.
The first parameter specifies the target container.
The following parameters specify the command and its arguments.
...
The function executes the specified command in the specified container
with the container's `USERNAME` account permissions.
The first parameter specifies the target container.
...
The function searches for the enclosing [Dsb root directory](#terms),
initializes [Dsb base variables](#base-variables),
...
Another call to the function after successful completion is simply ignored.
If you need to perform actions with several Dsb projects in one Dsb script,
you should use separate
[subshell blocks](https://tldp.org/LDP/abs/html/subshells.html)
to call `dsb_set_box` with a specific `--dir` option values.
...
The function can be used if containers of only one Dsb project are running
...
on failure, and the Dsb script can handle the failure:
    ...
    if ! dsb_set_single_box --check ; then
        dsb_set_box  # try the nearby Dsb project
    fi
    ...
...
The functions are used to display color messages:
