#
#   Run shell (bash or sh) or some command in the container with host user permissions
#
...
if [ "$DSB_UID" = 0 ]; then
    dsb_error_exit "$DSBLIB_BINCMD ${DSBLIB_DSBARG}: Cannot run in root mode!"
fi
...
if [ -n "$DSB_UID_GID" -a "$DSB_UID_GID" != "$MYUIDGID" ]; then
    echo "The container was launched from the different account ${DSB_UID_GID}!" 1>&2
    exit 126
fi
case "$TERM" in
    xterm-color | *-256color )
        export PS1='$( echo "\033[36m'"$MYSERVICE"':$PWD\033[m\$" ) '
        ;;
    *)
...
dsb_run_command "$MYSERVICEARG" "$DSB_UID_GID" - -l -c "dsbnop:MYSERVICE=${MYPSPREFIX@Q} ; MYUIDGID=${DSB_UID_GID@Q} ; ${MYINLINESCRIPT}" "dsbnop:$DSBLIB_BINCMD" "$@"
