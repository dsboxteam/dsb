...
case "$TERM" in
    xterm-color | *-256color )
        #if [ -n "$BASH" ]; then
        #    export PS1='\033[36m'"$MYSERVICE"':\w\033[m\$ '
        #else
...
dsblib_run_command   "$MYSERVICENAME" "$DSB_UID_GID" - -l -c "dsbnop:MYSERVICE=${MYPSPREFIX@Q} ; ${MYINLINESCRIPT}" "dsbnop:$DSBLIB_BINCMD" "$@"
dsblib_exit "$?"